...
generate_env_payload() {
  ...
  # Также убираем одинарные или двойные кавычки в начале и конце значений.
  grep -v '^\s*#' "$ENV_FILE" | grep -v '^\s*$' \
  | jq -R -n '[inputs | capture("(?<key>[^=]+)=(?<value>.*)") | {key: .key, text_value: (.value | gsub("^\"|\"$"; "") | gsub("^\\u0027|\\u0027$"; ""))}]'
}
